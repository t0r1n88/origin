...
class EmptyMap(MapFactory):
    ...
    def create_cell(i, j):
        ...
        :param i: длина
        :param j: ширина
        :return: wall или floor1 в зависимости от параметров i,j
        """
        # lim предел за который нельзя выходить
        lim = EmptyMap.map_size - 1
        return wall if (i == 0 or j == 0 or i == lim or j == lim) else floor1
    ...
    class Map:
        def __init__(self):
            ...
            Создаем карту
            """
            self.Map = [[EmptyMap.create_cell(i, j) for j in range(EmptyMap.map_size)] for i in range(EmptyMap.map_size)]
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            coord = (EmptyMap.map_size - 2, EmptyMap.map_size - 2)
...
class SpecialMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            # TODO Сделать нормальную генерацию лестиницы
            prop = object_list_prob['objects']['stairs']
            coord = (EmptyMap.map_size - 2, EmptyMap.map_size - 2)
            self.objects.append (Objects.Ally (prop['sprite'], prop['action'], coord))
            return self.objects
...
def service_init(sprite_size, full=True):
    ...
    # Если full будет равен False то списка object_list_prob не создаться и будет ошибка
    if full:

...
import Logic
import Service
# Спасибо Фроловой Екатерине Сергеевне за помощь =). Ее советы очень помогли
# Разрешение экрана
SCREEN_DIM = (800, 600)
# Инициализируем экран задаем название и контроль с помощью клавиатуры
pygame.init()
gameDisplay = pygame.display.set_mode(SCREEN_DIM)
pygame.display.set_caption("Телекинезис: Проклятие Шаммуррат")
KEYBOARD_CONTROL = True

if not KEYBOARD_CONTROL:
    import numpy as np
    answer = np.zeros(4, dtype=float)
# Словарь хранящий базовые характеристики нашего героя
base_stats = {
    "strength": 20,
    "endurance": 20,
    "intelligence": 5,
...
def create_game(sprite_size:int, is_new:bool):
    ...
    :param sprite_size: размер спрайта героя
    :param is_new: ключ определяющий создание новой игры(вроде бы)
    но зачем этот ключ и что мы должны делать в другом случае?
    :return:
    """
    # Делаем глобальные переменные
    # Вопрос на засыпку а что мы делаем в ветви else?
    ...
    if is_new:
        ...
        # создаем движок самой игры
        engine = Logic.GameEngine()
        # Забираем данные из yaml файлов levels и objects, а также загружаем текстуры, создаем список действий
        ...
        # Надписи FIX ME тут нет, но код который расположен здесь не может работать
        # потому что переменные которые обьявлены глобальными, создаются в другой ветке условного оператора
